...
class TestConsolidationElement1D(unittest.TestCase):
    def setUp(self):
        self.nodes = tuple(Node1D(k, 2.0 * k + 1.0) for k in range(2))
        self.e = Element1D(self.nodes, order=1)
        self.thrm_e = ConsolidationElement1D(self.e)
    ...
    def test_jacobian_equal(self):
        self.assertAlmostEqual(self.e.jacobian, self.cond_e.jacobian)

    def test_nodes_equal(self):
        for nd, e_nd in zip(self.nodes, self.cond_e.nodes):
...
class TestConsolidationBoundary1D(unittest.TestCase):
    ...
    def test_invalid_parent(self):
        with self.assertRaises(TypeError):
            ThermalBoundary1D(self.nodes, order=1)
