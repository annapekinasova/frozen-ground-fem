...
class TestConsolidationElement1DCubic(unittest.TestCase):
    ...
    def test_stiffness_matrix_uninitialized(self):
        # WHY THIS TEST IS DIFFERENT FOR MASS MATRIX? Line 178
        self.assertTrue(np.allclose(
    ...
    def test_stiffness_matrix(self):
        ...
        coef_1 = (dk_de * (Gs - 1.0) / (1.0 + e)
                  - k * (Gs - 1.0) / (1.0 + e) ** 2)
        stiff_0 = coef_0 * 1/40 * np.array([[148, -189, 54, -13],
                                            [-189, 432, -297, 54],
                                            [54, -297, 432, -189],
                                            [-13, 54, -189, 148]])
        stiff_1 = coef_1 * 1/1680 * np.array([[-840, 1197, -504, 147],
                                              [-1197, 0, 1701, -504],
                                              [504, -1701, 0, 1197],
                                              [-147, 504, -1197, 840]])
        expected = stiff_0 + stiff_1
